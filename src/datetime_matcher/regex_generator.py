...
class RegexGenerator():
    ...
    def __generate_parts_from_dfregex_tokens(self, tokens: Iterable[DfregexToken], is_capture_dfs: bool) -> Iterator[str]:
        ...
        counter = 0
        for token in tokens:
            result = self.__generate_part_from_dfregex_token(token, is_capture_dfs, counter)
            yield result
            if token.kind == 'DATETIME_FORMAT_CODE':
                counter += 1

    # private
    def __generate_part_from_dfregex_token(self, token: DfregexToken, is_capture_dfs: bool, num_format_codes_encountered: int) -> str:
        """
        Parse a DfregexToken into a string which makes a part of a regex pattern.
        """
        if token.kind == 'DATETIME_FORMAT_CODE':
            result = self.__get_regex_from_format_code(cast(SupportedDatetimeFormatCodeType, token.value[1:]), is_capture_dfs, num_format_codes_encountered)
    ...
    # private
    def __get_regex_from_format_code(self, format_code: SupportedDatetimeFormatCodeType, is_capture_dfs: bool, capture_dfs_idx: int) -> Optional[str]:
